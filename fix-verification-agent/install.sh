...
usage() {
    ...
    echo ""
    echo "Install the Fix Verification Agent into the Claude Agents virtual environment."
    echo ""
    echo "Options:"
...
if [ -z "$INSTALL_SYSTEMD" ]; then
    echo -n "Install systemd service files for the fix verification agent? [y/N] "
    read -r _response
...
if [ "$INSTALL_SYSTEMD" = "yes" ]; then
    ...
    echo ""
    echo "To enable the fix verification agent:"

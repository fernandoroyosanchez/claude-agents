...
You are the Code Review Agent for the OpenStack projects.
...
```bash
ls ~/.venv/claude-agents/bin/openstack-review-change 2>/dev/null || echo "NOT INSTALLED — run ./setup-agents.sh first"
...
~/.venv/claude-agents/bin/openstack-review-change <change_number>
```

**Review a specific patchset**:
```bash
~/.venv/claude-agents/bin/openstack-review-change <change_number> <patchset>
```

**Review by Gerrit URL**:
```bash
~/.venv/claude-agents/bin/openstack-review-change https://review.opendev.org/c/openstack/octavia/+/982567
```

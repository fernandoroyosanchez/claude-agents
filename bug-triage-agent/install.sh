...
usage() {
    ...
    echo ""
    echo "Install the Bug Triage Agent into the Claude Agents virtual environment."
    echo ""
    echo "Options:"
...
echo -e "${GREEN}✓${NC} bug-triage-agent installed (openstack-triage-bugs)"
...
if [ -z "$INSTALL_SYSTEMD" ]; then
    echo -n "Install systemd service files for the bug triage agent? [y/N] "
    read -r _response
...
if [ "$INSTALL_SYSTEMD" = "yes" ]; then
    ...
    echo ""
    echo "To enable the bug triage agent:"

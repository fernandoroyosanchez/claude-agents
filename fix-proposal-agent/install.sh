...
usage() {
    ...
    echo ""
    echo "Install the Fix Proposal Agent into the Claude Agents virtual environment."
    echo ""
    echo "Options:"
...
echo -e "${GREEN}✓${NC} fix-proposal-agent installed (openstack-propose-fix)"
...
if [ -z "$INSTALL_SYSTEMD" ]; then
    echo -n "Install systemd service files for the fix proposal agent? [y/N] "
    read -r _response
...
if [ "$INSTALL_SYSTEMD" = "yes" ]; then
    ...
    echo ""
    echo "To enable the fix proposal agent:"
